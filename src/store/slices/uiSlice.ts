...
import { SidebarTab, UserSettings, PendingCloseFile } from '../../types/ide'
...
export const createUISlice: StateCreator<FullIDEStore, [], [], UISlice> = (set, get) => ({
  ...
  setSidebarWidth: (w) => set({
    sidebarWidth: Math.max(LAYOUT_CONSTRAINTS.SIDEBAR_MIN_WIDTH, Math.min(w, LAYOUT_CONSTRAINTS.SIDEBAR_MAX_WIDTH))
  }),

  setTerminalHeight: (h) => set({
    terminalHeight: Math.max(LAYOUT_CONSTRAINTS.TERMINAL_MIN_HEIGHT, Math.min(h, LAYOUT_CONSTRAINTS.TERMINAL_MAX_HEIGHT))
  }),

  setZoomLevel: (updater) => set((state) => {
    const next = typeof updater === 'function' ? updater(state.zoomLevel) : updater
    return {
      zoomLevel: Math.max(LAYOUT_CONSTRAINTS.ZOOM_MIN, Math.min(LAYOUT_CONSTRAINTS.ZOOM_MAX, Math.round(next * 100) / 100))
    }
  }),
